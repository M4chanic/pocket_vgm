// m4pocket: константы savestate-регистров перенесены сюда из
// regs_savestates.sv (Verilator не разделяет $unit между файлами)
...
module DmcChan #(parameter [9:0] SSREG_INDEX_DMC1, parameter [9:0] SSREG_INDEX_DMC2)  (
	...
	// m4pocket: dma_address разделён для Verilator (BLKANDNBLK)
	logic [14:0] dma_addr_lo;
	assign dma_address = {1'b1, dma_addr_lo};
	...
	always_ff @(posedge clk) begin
			...
			case (ain)
				...
				5: begin // $4015
						...
						if (DIN[4] && ~enable) begin
							dma_addr_lo <= {1'b1, sample_address[7:0], 6'h00};
		...
		if (aclk1_d) begin // Put cycle
			...
			if (dma_ack) begin
				...
				if (|bytes_remaining)
					...
					dma_addr_lo <= {1'b1, sample_address[7:0], 6'h0};
					bytes_remaining <= {sample_length, 4'h0};
					enable <= loop;
					if (~loop & irq_enable)
						irq <= 1;
		...
		if (reset) begin
			...
			dma_addr_lo <= SS_DMC2[14: 0]; // 15'h0000;
	...
	assign SS_DMC2_BACK[14: 0] = dma_addr_lo;
