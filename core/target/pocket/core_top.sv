...
module core_top (
  ...
  // cram0 занят под сэмпл-ROM chipbox (см. psram sample_ram ниже)
  ...
  // Какой data-слот Pocket обновил последним (музыкальные слоты 1..N):
  // единственный сигнал «какой Load нажал пользователь». Латч в clk_74a,
  // квазистатичен к моменту чтения фирмварью — 2FF-синхронизация ниже.
  reg [15:0] last_upd_id = 16'hFFFF;
  reg [7:0] upd_count = 0;
  ...
  always @(posedge clk_74a) begin
    if (~pll_core_locked) begin
      ...
      // ВАЖНО: неблокирующее — блокирующее убивало детект фронта ниже
      // (prev успевал сравняться с текущим значением до проверки)
      ...
      if (apf_bridge_file_size_wr_s) begin
        ...
        if (dataslot_update_id != 0) begin
          last_upd_id <= dataslot_update_id;
          upd_count <= upd_count + 1'b1;
        end
      end
    end
  end
  ...
  // Внешний Wishbone-регион LiteX (0x8000_0000) — chipbox
  wire [29:0] wb_addr;
  wire [1:0] wb_bte;
  wire [2:0] wb_cti;
  wire wb_cyc;
  ...
  // Аудио звуковых чипов (домен clk_sys)
  wire signed [15:0] chip_left;
  wire signed [15:0] chip_right;
  wire chip_sample_toggle;

  // Память сэмплов chipbox (PSRAM банк 0)
  wire chip_mem_rd;
  wire chip_mem_wr;
  wire [21:0] chip_mem_addr;
  wire [15:0] chip_mem_wdata;
  wire [1:0] chip_mem_wbe;
  wire [15:0] chip_mem_rdata;
  wire chip_mem_rdata_valid;
  wire chip_mem_busy;
  ...
  litex litex (
      ...
      .wishbone_ack(wb_ack),
      .wishbone_adr(wb_addr),
      .wishbone_bte(wb_bte),
      .wishbone_cti(wb_cti),
      .wishbone_cyc(wb_cyc),
      .wishbone_dat_r(wb_data_read),
      .wishbone_dat_w(wb_data_write),
      .wishbone_err(wb_err),
      .wishbone_sel(wb_sel),
      .wishbone_stb(wb_stb),
      .wishbone_we(wb_we),
  ...
  reg [23:0] slot_upd_s1 = 24'hFF_FFFF;
  reg [23:0] slot_upd_s2 = 24'hFF_FFFF;

  always @(posedge clk_sys_57_12) begin
    slot_upd_s1 <= {upd_count, last_upd_id};
    slot_upd_s2 <= slot_upd_s1;
  end

  chipbox chipbox (
      .clk(clk_sys_57_12),

      .slot_upd_info(slot_upd_s2),
      ...
      .chip_left(chip_left),
      .chip_right(chip_right),
      .chip_sample_toggle(chip_sample_toggle),

      .mem_rd(chip_mem_rd),
      .mem_wr(chip_mem_wr),
      .mem_addr(chip_mem_addr),
      .mem_wdata(chip_mem_wdata),
  ...
  psram #(
      ...
      .write_en(chip_mem_wr),
      .data_in(chip_mem_wdata),
      .write_high_byte(chip_mem_wbe[1]),
      .write_low_byte(chip_mem_wbe[0]),

      .read_en(chip_mem_rd),
      .read_avail(chip_mem_rdata_valid),
      .data_out(chip_mem_rdata),
